...
export default function CartPage({ navigate, cart, onUpdateQty, onRemove }) {
  ...
  const total = subtotal - discount + (subtotal > 999 || subtotal === 0 ? 0 : 99);
  ...
  const applyCoupon = () => {
    if (coupon.toUpperCase() === "ORGANIC10") {
      ...
      setDiscount(Math.min(200, subtotal));
                ...
                <SectionCard key={item.id} style={{ marginBottom: 16, padding: "20px 24px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", border: `1.5px solid ${T.border}`, borderRadius: 50, overflow: "hidden" }}>
                      <button onClick={() => onUpdateQty(item.id, -1)} style={{ width: 32, height: 32, background: "transparent", border: "none", cursor: "pointer", fontSize: 16 }}>−</button>
                    ...
                    <div style={{ textAlign: "right", minWidth: 100 }}>
                      ...
                      <button onClick={() => onRemove(item.id)} style={{ background: "none", border: "none", color: "#e74c3c", fontSize: 12, fontWeight: 600, cursor: "pointer", padding: 0 }}>Remove</button>
                    </div>
                  </div>
                </SectionCard>
              ))}
            ...
            <div style={{ position: "sticky", top: 100 }}>
              <SectionCard style={{ padding: 28 }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 14, fontSize: 14, color: T.textMid }}>
                  ...
                  <span style={{ fontWeight: 600, color: subtotal > 999 ? T.green : T.textDark }}>
                    {subtotal > 999 ? "FREE" : subtotal === 0 ? "₹0" : "₹99"}
                  </span>
                </div>
              ...
              {subtotal > 0 && subtotal < 999 && (
                <div style={{ marginTop: 16, background: T.greenPale, borderRadius: 12, padding: 16, border: `1px solid ${T.border}`, display: "flex", gap: 12, alignItems: "center" }}>
                  ...
                  <span style={{ fontSize: 12, color: T.green, fontWeight: 500 }}>You are ₹{999 - subtotal} away from <strong>FREE delivery</strong>!</span>
                </div>
              )}
            </div>
          </div>
